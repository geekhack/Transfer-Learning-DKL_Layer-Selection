...
for layer in model.layers[0:-2]:
    ...
    if ('project' in layer.name) and ('project_BN' not in layer.name):
        convolved_layers.append(index)
        convolved_layer_names.append(layer)
    if ('conv' in layer.name) and ('conv_depthwise_BN' not in layer.name) and (
            'conv_project_BN' not in layer.name) and ('conv_depthwise_relu' not in layer.name):
        convolved_layers.append(index)
        convolved_layer_names.append(layer)
...
for lyr in convolved_layer_names:
    ...
    if len(ary) != 0:
        ...
        jdk_pos.update({layer_index: layer_softmax_values})
...
# for ngs
with open('mobilenetv2_pure_kullback_positive_neg.csv', 'w', encoding='UTF8', newline='') as f:
